...
class IOConnection implements IOCallback {
  ...
  private void handshake() {
    ...
    try {
      ...
      url =
          new URL(IOConnection.this.url.getProtocol() + "://"
              + IOConnection.this.url.getAuthority() + SOCKET_IO_1 + "?"
              + IOConnection.this.url.getQuery()
          );
  ...
  private synchronized void connectTransport() {
    ...
    if (protocols.contains(WebsocketTransport.TRANSPORT_NAME)) {
      try {
        transport = WebsocketTransport.create(new URL(url.getProtocol() + "://"
            + url.getAuthority()), this);
