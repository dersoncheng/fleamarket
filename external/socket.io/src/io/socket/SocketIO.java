...
public class SocketIO {
   ...
   * calling {@link #connect(URL, IOCallback)} or {@link #connect(String, IOCallback)}
   */
  public SocketIO() {
   ...
   * instantiate {@link SocketIO} using {@link #SocketIO(String)} or {@link #SocketIO(URL)}.
   * 
   * @param callback
   *          the callback
   */
  ...
  private boolean setAndConnect(URL url, IOCallback callback) {
    ...
    if (this.callback != null && this.url != null) {
      final String origin = this.url.getProtocol() + "://"
          + this.url.getAuthority() + "?" + this.url.getQuery();
