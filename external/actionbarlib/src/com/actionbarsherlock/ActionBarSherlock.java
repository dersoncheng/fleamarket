 ...
 * 
 * <p>
 * This class will manage interaction with a custom action bar based on the Android 4.0 source code.
 * The exposed API mirrors that of its native counterpart and you should refer to its documentation
 * for instruction.
 * </p>
 * 
...
public abstract class ActionBarSherlock {
  ...
  /**
   * <p>
   * Denotes an implementation of ActionBarSherlock which provides an action bar-enhanced
   * experience.
   * </p>
   */
  ...
  /**
   * If set, the logic in these classes will assume that an {@link Activity} is dispatching all of
   * the required events to the class. This flag should
   ...
   * 
   * @param implementationClass Target implementation class which extends {@link ActionBarSherlock}.
   *          This class must also be annotated with {@link Implementation}.
   */
  public static void registerImplementation(Class<? extends ActionBarSherlock> implementationClass) {
    if (!implementationClass.isAnnotationPresent(Implementation.class)) {
      throw new IllegalArgumentException("Class " + implementationClass.getSimpleName()
  ...
  public static ActionBarSherlock wrap(Activity activity, int flags) {
    ...
    if (hasQualfier) {
      final boolean isTvDpi =
          activity.getResources().getDisplayMetrics().densityDpi == DisplayMetrics.DENSITY_TV;
   ...
   *          contains the data it most recently supplied in {@link Activity#}
  ...
  /**
   * Enable extended screen features. This must be called before {@code setContentView()}. May be
   * called as many times as desired as long
  ...
  /**
   * Variation on {@link #setContentView(android.view.View, android.view.ViewGroup.LayoutParams)} to
   * add an additional content view to the screen. Added after any
   ...
   * progress is drawn between the primary progress (set via {@link #setProgress(int)} and the
   * background. It can be ideal for media
